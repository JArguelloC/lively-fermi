 ...
 */
function generateDiscountPrice(originalPrice) {
 ...
 */
export async function generateOffersInFirestore() {
    const productsRef = collection(db, 'products');
    const batch = writeBatch(db);
    let offersCreated = 0;
    const errors = [];
        ...
        for (const category of categories) {
            ...
            for (const product of selectedForOffers) {
                try {
                    ...
                    offersCreated++;
                    console.log(`✨ Oferta creada: ${product.name} | ` +
 ...
 */
export async function seedNewsToFirestore() {
