 ...
 */
export async function checkProductStockAvailability(productId, quantity) {
    try {
 ...
 */
export async function validateOrderStock(cartItems) {
    const errors = [];
    const itemDetails = [];
    try {
 ...
 */
export async function decrementProductStock(cartItems) {
    ...
    try {
        ...
        const result = await runTransaction(db, async (transaction) => {
            ...
            const productDocs = await Promise.all(cartItems.map(item => transaction.get(doc(db, 'products', item.id))));
            const updatesQueue = [];
            ...
            for (let i = 0; i < cartItems.length; i++) {
                ...
                if (!productSnap.exists()) {
                    throw new Error(`❌ ROLLBACK: Producto '${item.name}' (${item.id}) no existe en inventario. ` +
                ...
                if (currentStock < item.quantity) {
                    throw new Error(`❌ ROLLBACK: Stock insuficiente para '${item.name}'. ` +
 ...
 */
export async function getCurrentProductStock(productId) {
    try {
        const docRef = doc(db, 'products', productId);
        const docSnap = await getDoc(docRef);
        if (!docSnap.exists()) {
